...
void printGameBoard(int grid[][10]) //This is the board that is printed for playing the game.  You cannot see the boats
{
    ...
    for (int i = 0; i < 10; i++)  //column loop
    {
        for (int j = 0; j < 10; j++)  //row loop
        {
            ...
            if (grid[i][j] == 5) cout << 5; //if the space is a hit, print it           
            else if (grid[i][j] == 9) cout << 9; //if the space is a miss, print it           
...
//This function places the individual boats onto the initialised grid
{
    ...
    while (placementFailure)
    {
        if (dir == 'h')
        {
            cS = checkSpaces(grid, col, row, BoatSize, dir);//check to make sure the boat can be placed without overlapping another boat
            ...
            editGrid(grid, col, row, BoatSize, dir);//place the boat on the grid
            editBoatInfo(grid, col, row, BoatSize, dir, boatList, name);//create the boat object
            return 0;
...
int checkSpaces(int grid[][10], int col, int row, int size, char d)
...
void editBoatInfo(int grid[][10], int col, int row, int BoatSize, char d, vector<Boat>& boatList, int name)
